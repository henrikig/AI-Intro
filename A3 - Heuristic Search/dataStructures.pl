...
openNode_to_ClosedNode(OpenNode, ClosedNode) :-
    ...
    make_closedNode([state(State), gValue(GValue), fValue(FValue), parent(Parent)], ClosedNode).
...
expand_OpenNode(OpenNode, Actions, SuccessorNodes) :-
    ...
    findall(ChildNode,
       ...
       h(ChildState, Actions, HValue),
       ChildFValue is ChildGValue + HValue,
...
:- record closedNode(state, gValue, fValue, parent).
